...
new class extends Component {
    ...
    // Stocker les produits affichés pour les utiliser dans les méthodes
    ...
    {
        // Cette méthode sera mise à jour après le rendu avec les données réelles
        $this->dispatch('update-select-all', ['value' => $value]);
    }
    
    // Écouter l'événement pour mettre à jour la sélection avec les produits actuels
    #[On('update-select-all')]
    public function handleSelectAllUpdate($value)
    {
        // Utiliser les produits actuellement affichés stockés dans $this->displayedProducts
        if (empty($this->displayedProducts)) {
            return;
        }
        
        if ($value) {
            // Sélectionner tous les produits visibles
            $visibleSkus = collect($this->displayedProducts)->pluck('sku')->toArray();
            $visibleDetails = collect($this->displayedProducts)->map(function($product) {
                return [
                    'sku' => $product['sku'] ?? '',
                    'title' => $product['title'] ?? '',
