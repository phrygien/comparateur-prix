...
Exemple 4 - Produit : \"Lancôme - La Nuit Trésor Rouge Drama - Eau de Parfum Intense Vaporisateur 30ml\"
{
  ...
  \"is_coffret\": false
    ...
    private function searchMatchingProducts()
    {
        ...
        // ✨ SKIP pour Guerlain : on applique directement le filtrage strict plus tard
        $nameFilteredProducts = $filteredProducts;
        
        if ($isGuerlainProduct && !empty($nameWords)) {
            \Log::info('⚠️ GUERLAIN - Skip du filtrage progressif par NAME', [
                'raison' => 'Guerlain nécessite un filtrage strict à 100%',
                'filtrage_strict_appliqué_à' => 'ÉTAPE 2.67',
                'produits_conservés' => count($filteredProducts)
            ]);
        }
        
        if (!empty($nameWords) && !$isGuerlainProduct) {
        ...
        // ✅ ÉTAPE 2.64 : FILTRAGE STRICT pour Guerlain (PRIORITAIRE - appliqué en PREMIER)
        if ($isGuerlainProduct && !empty($filteredProducts)) {
            ...
            if (!empty($guerlainFiltered)) {
                \Log::info('✅ GUERLAIN - Filtrage strict appliqué (100% des mots + Flacon)', [
                    'produits_avant' => count($filteredProducts),
                    'produits_après' => count($guerlainFiltered),
                    'nom_recherché' => $name,
                    'type_recherché' => $type,
                    'mots_requis' => $this->extractKeywords($name, false)
                ]);
