...
/* ========================================
   EXTRAITS DE CODE POUR GUERLAIN
   À copier-coller dans votre fichier PHP
   ======================================== */

/* ----------------------------------------
   1. NOUVELLE FONCTION : isGuerlainProduct()
   Localisation : Après isHermesProduct()
   ---------------------------------------- */
...
/* ----------------------------------------
   2. NOUVELLE FONCTION : isValidGuerlainMatch()
   Localisation : Après isValidHermesMatch()
   ---------------------------------------- */
 ...
 * 1. TOUS les mots significatifs du NAME recherché doivent être présents
...
private function isValidGuerlainMatch(string $searchName, string $searchType, string $productName, string $productType): bool
{
    ...
    // Vérification STRICTE du NAME - Tous les mots doivent être présents (100%)
    $matchCount = 0;
    $matchedWords = [];
    $missingWords = [];
...
/* ----------------------------------------
   3. MODIFICATION : isSpecialVendor()
   Ajouter 'guerlain' à la liste
   ---------------------------------------- */

private function isSpecialVendor(string $vendor): bool
{
    $specialVendors = ['valentino', 'valent', 'hermès', 'hermes', 'guerlain']; // ← Ajouter 'guerlain'
    $vendorLower = mb_strtolower(trim($vendor));
...
/* ----------------------------------------
   4. AJOUT dans searchMatchingProducts()
   Après la ligne : $isHermesProduct = ...
   ---------------------------------------- */
...
/* ----------------------------------------
   5. MODIFICATION : Log de détection
   Dans le if ($isSpecialVendor || $isMeteoritesProduct)
   ---------------------------------------- */
...
\Log::info('🎯 PRODUIT SPÉCIAL DÉTECTÉ', [
    ...
    'is_valentino' => str_contains(mb_strtolower($vendor), 'valent'),
    'is_hermes' => $isHermesProduct,
    'is_guerlain' => $isGuerlainProduct, // ← Ajouter cette ligne
    'is_barenia' => $isBareniaProduct,
    'is_meteorites' => $isMeteoritesProduct,
    'is_coffret' => $isCoffretSource,
    'is_limited_edition' => $isLimitedEdition
...
/* ----------------------------------------
   6. NOUVEAU BLOC : Filtrage Guerlain (ÉTAPE 2.67)
   Localisation : Après le bloc Hermès (ÉTAPE 2.66)
   ---------------------------------------- */
...
/* ----------------------------------------
   7. AJOUT dans le SCORING
   Dans la fonction map() du scoring
   Localisation : Après le bonus Hermès
   ---------------------------------------- */
