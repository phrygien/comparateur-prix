...
<div class="w-full px-4 py-2 sm:px-2 sm:py-4 lg:grid lg:grid-cols-2 lg:gap-x-8 lg:px-8">
    ...
    <div class="mt-10 lg:col-start-1 lg:row-span-2 lg:mt-0 lg:self-center">
        @if(isset($product['data'][0]->media_gallery) && !empty($product['data'][0]->media_gallery))
            @php
        ...
        @elseif(isset($product['data'][0]->thumbnail))
            <img src="{{ $product['data'][0]->thumbnail }}" alt="{{ $product['data'][0]->title }}" class="aspect-square w-full rounded-lg object-cover">
        @else
    ...
    <div class="lg:max-w-lg lg:self-end lg:col-start-2">
        ...
        <div class="mt-4">
            <h1 class="text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">
                {{ $product['data'][0]->title ?? 'Titre du produit' }}
            </h1>
        </div>
            ...
            @if(isset($product['data'][0]->special_price) && $product['data'][0]->special_price < $product['data'][0]->price)
                <div class="flex items-center gap-3">
                    <p class="text-2xl font-bold text-gray-900">
                        {{ number_format($product['data'][0]->special_price, 2, ',', ' ') }} €
                    </p>
                    <p class="text-lg text-gray-500 line-through">
                        {{ number_format($product['data'][0]->price, 2, ',', ' ') }} €
                    </p>
                    @php
        ...
        <section aria-labelledby="information-heading" class="mt-4">
            ...
            <div class="mt-4 space-y-6">
                @if(isset($product['data'][0]->short_description))
                    <p class="text-base text-gray-500">{{ $product['data'][0]->short_description }}</p>
                @elseif(isset($product['data'][0]->description))
                    <p class="text-base text-gray-500">{{ Str::limit(strip_tags($product['data'][0]->description), 200) }}</p>
                @endif
            </div>
            ...
            <div class="mt-6 flex items-center">
                @if($product['data'][0]->quatity_status && $product['data'][0]->quatity > 0)
                    ...
                    <p class="ml-2 text-sm text-gray-500">
                        En stock ({{ $product['data'][0]->quatity }} unités)
                    </p>
                @else
                    <svg class="size-5 shrink-0 text-red-500" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
            ...
            <div class="mt-4 space-y-2">
                ...
                @endif
                @if(isset($product['data'][0]->option_value))
                    <p class="text-xs text-gray-500">Capacité: {{ $product['data'][0]->option_value }}</p>
                @endif
            </div>
        </section>
    </div>
    ...
    <div class="mt-10 lg:col-start-2 lg:row-start-2 lg:max-w-lg lg:self-start">
        <section aria-labelledby="options-heading">
            ...
            <form>
                ...
                <!-- Add to cart button -->
                <div class="mt-6">
                    <button type="submit" class="flex w-full items-center justify-center rounded-md border border-transparent bg-indigo-600 px-8 py-3 text-base font-medium text-white hover:bg-indigo-700 focus:outline-hidden focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
                        Ajouter au panier
                    </button>
                </div>
            </form>
