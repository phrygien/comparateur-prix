...
new class extends Component {
    ...
    public function getCompetitorPrice($search)
    {
        try {
            ...
            \Log::info('Recherche SQL optimisée:', [
    ...
    private function buildOptimizedSearchQuery(): array
    {
        ...
        // 1. Recherche par vendor (priorité haute)
        if (!empty($this->extractedData['vendor'])) {
            ...
            foreach ($vendorKeywords as $keyword) {
                $conditions[] = "(vendor LIKE ? OR name LIKE ?)";
                $params[] = "%{$keyword}%";
                $params[] = "%{$keyword}%";
            }
        }
        
        // 2. Recherche par nom du produit
        ...
        // 3. Recherche par volume (priorité haute)
        if (!empty($this->extractedData['volume_ml'])) {
            $conditions[] = "(name LIKE ? OR variation LIKE ?)";
            $params[] = "%{$this->extractedData['volume_ml']} ml%";
            $params[] = "%{$this->extractedData['volume_ml']}ml%";
        }
        ...
        if (!empty($this->extractedData['vendor'])) {
            $orderConditions[] = "vendor = '" . addslashes($this->extractedData['vendor']) . "' DESC";
        }
        
        // Priorité 2: Correspondance du volume
        if (!empty($this->extractedData['volume_ml'])) {
            $orderConditions[] = "(name LIKE '%{$this->extractedData['volume_ml']} ml%' OR variation LIKE '%{$this->extractedData['volume_ml']}ml%') DESC";
        }
        
        // Priorité 3: Correspondance du type
        if (!empty($this->extractedData['type'])) {
            $orderConditions[] = "type LIKE '%" . addslashes($this->extractedData['type']) . "%' DESC";
        }
        
        // Priorité 4: Correspondance du nom
        if (!empty($this->extractedData['name'])) {
            $orderConditions[] = "name LIKE '%" . addslashes($this->extractedData['name']) . "%' DESC";
        }
    ...
    private function extractKeywordsFromVendor(string $vendor): array
    {
        ...
        foreach ($words as $word) {
            $word = trim($word);
            if (!empty($word) && !in_array(mb_strtolower($word), $stopWords)) {
                $keywords[] = $word;
            }
        }
        
        // Ajouter aussi le nom complet du vendeur
        if (count($keywords) > 1) {
            $keywords[] = $vendor;
        }
    ...
    private function extractTypeKeywords(string $type): array
    {
        ...
        // Ajouter des variations du type
        $typeVariations = [
    ...
    private function extractKeywords(string $text): array
    {
        ...
        foreach ($words as $word) {
            $word = trim($word);
            if (!empty($word) && strlen($word) > 1 && !in_array(mb_strtolower($word), $stopWords)) {
                $keywords[] = $word;
            }
        }
    ...
    /**
     * Prépare les termes de recherche pour FULLTEXT
     */
