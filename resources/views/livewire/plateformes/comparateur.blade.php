...
new class extends Component {
    ...
    private function extractSearchComponents(string $search): array
    {
        ...
        if (!$hasProductKeyword) {
            ...
            if (!empty($vendor) && isset($parts[1])) {
                $remainingSearch = $parts[1];
            }
        }
        
        // Si pas trouvé, chercher dans toute la recherche
        if (empty($vendor)) {
            foreach ($this->knownVendors as $knownVendor) {
                $position = mb_stripos(mb_strtolower($search), mb_strtolower($knownVendor));
                if ($position !== false && $position < 30) {
                    $vendor = $knownVendor;
                    $remainingSearch = trim(str_ireplace($vendor, '', $search));
        ...
        $components['product_name'] = trim($remainingSearch);
        
        // Extraire le type
        $types = [
            'eau de parfum', 'eau de toilette', 'eau fraiche', 'eau de cologne',
            'parfum', 'edp', 'edt', 'edc', 'coffret'
        ];
        
        foreach ($types as $type) {
            if (stripos(mb_strtolower($remainingSearch), $type) !== false) {
    ...
    public function getCompetitorPrice($search)
    {
        try {
            ...
            // ÉTAPE 1 : Recherche complète avec le nom
            $searchQuery = $this->prepareAutomaticSearchTerms($search);
            
            $allProducts = [];
            
            // Première recherche : recherche complète
            ...
            \Log::info('Full search results:', ['count' => count($fullSearchProducts)]);

            // Si peu de résultats avec la recherche complète, essayer une recherche par vendor
            if (count($fullSearchProducts) < 5 && !empty($this->filters['vendor'])) {
                \Log::info('Too few results, trying vendor search for:', ['vendor' => $this->filters['vendor']]);
                
                // ÉTAPE 2 : Recherche par vendor uniquement
                $vendorVariations = $this->getVendorVariations($this->filters['vendor']);
                $vendorConditions = [];
                $vendorParams = [];
                ...
                foreach ($vendorSearchProducts as $product) {
                ...
                $allProducts = $fullSearchProducts;
            }

            \Log::info('Total products after search:', ['count' => count($allProducts)]);
            ...
            foreach ($processedProducts as $product) {
                ...
                if ($similarityScore >= $tempThreshold) {
                    ...
                    $product->search_source = count($fullSearchProducts) < 5 ? 'vendor_search' : 'full_search';
            ...
            usort($matchedProducts, function ($a, $b) {
                // Priorité aux résultats de recherche complète, puis par score
                if ($a->search_source === 'full_search' && $b->search_source !== 'full_search') {
                    return -1;
                } elseif ($a->search_source !== 'full_search' && $b->search_source === 'full_search') {
                    return 1;
                }
            ...
            $fullResult = [
                ...
                'search_strategy' => count($fullSearchProducts) < 5 ? 'vendor_fallback' : 'full_search'
            ];
    ...
    /**
     * Méthode de similarité AMÉLIORÉE
     */
    private function computeOverallSimilarityImproved($product, $search, $components)
    {
        ...
        // 1. Score du nom (amélioré pour chercher des parties du nom)
        ...
        // 3. Score de la variation (simplifié)
...
<div>
    ...
    <table class="min-w-full border-collapse border border-gray-300">
                    ...
                    <!-- Mobile: Cartes (jusqu'à md) -->
                    <div class="md:hidden space-y-4">
                        @if(count($matchedProducts) > 0)
                            @foreach($matchedProducts as $index => $product)
                                @php
                                    // Mêmes calculs que pour le tableau
                                    if ($isAutomaticSearch) {
                                        $similarityScore = $product->similarity_score ?? null;
                                        $matchLevel = $product->match_level ?? null;
                                    } else {
                                ...
                                <div class="bg-white rounded-lg shadow-md border border-gray-300 p-4">
                                    ...
                                    <div class="flex items-start space-x-4">
                                        ...
                                        <div class="flex-shrink-0">
                                            <img src="{{ $productImage }}" 
                                                alt="{{ $productName }}" 
                                                class="h-24 w-24 object-cover border border-gray-300 rounded-lg"
                                                loading="lazy"
                                                onerror="this.onerror=null; this.src='https://placehold.co/400x400/cccccc/999999?text=No+Image'">
                                        </div>
                                        ...
                                        <div class="flex-1">
                                            ...
                                            <!-- Score et Correspondance -->
                                            @if($hasData && $isAutomaticSearch && $similarityScore)
                                            <div class="flex items-center space-x-4 mb-2">
                                                ...
                                                <!-- Correspondance -->
                                                @if($matchLevel)
                                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border {{ $matchClass }}">
                                                    {{ ucfirst($matchLevel) }}
                                                </span>
                                                @endif
                                            </div>
                                    ...
                                    <div class="mt-4 space-y-3">
                                        <!-- Variation -->
                                        <div class="flex items-center justify-between">
                                            <span class="text-xs text-gray-500">Variation:</span>
                                        ...
                                        <!-- Prix HT -->
                                        <div class="flex items-center justify-between">
                                            <span class="text-xs text-gray-500">Prix HT:</span>
                                        ...
                                        <!-- Date MAJ -->
                                        <div class="flex items-center justify-between">
                                            <span class="text-xs text-gray-500">Dernière MAJ:</span>
                                        ...
                                        <!-- Comparaison de prix -->
                                        @if($referencePrice && is_numeric($competitorPrice) && is_numeric($referencePrice))
                                        <div class="pt-3 border-t border-gray-200">
                                            <!-- Vs Cosmaparfumerie -->
                                            <div class="mb-2">
                                                <div class="flex items-center justify-between mb-1">
                                                    <span class="text-xs font-medium text-gray-700">Vs Cosmaparfumerie:</span>
                                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium {{ $priceStatusClass }}">
                                                        {{ $priceStatusLabel }}
                                                    </span>
                                                </div>
                                                <div class="flex items-center justify-between">
                                                    <span class="text-xs text-gray-500">Différence:</span>
                                                    <span class="text-xs font-semibold {{ $priceDifference > 0 ? 'text-green-600' : ($priceDifference < 0 ? 'text-red-600' : 'text-blue-600') }}">
                                                        {{ $this->formatPriceDifference($priceDifference) }}
                                                    </span>
                                                </div>
                                            </div>
                                            ...
                                            <!-- Vs Cosmashop -->
                                            @if(is_numeric($cosmashopPrice))
                                            <div>
                                                <div class="flex items-center justify-between mb-1">
                                                    <span class="text-xs font-medium text-gray-700">Vs Cosmashop:</span>
                                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium {{ $cosmashopStatusClass }}">
                                                        {{ $cosmashopStatusLabel }}
                                                    </span>
                                                </div>
                                                <div class="flex items-center justify-between">
                                                    <span class="text-xs text-gray-500">Différence:</span>
                                                    <span class="text-xs font-semibold {{ $cosmashopDifference > 0 ? 'text-green-600' : ($cosmashopDifference < 0 ? 'text-red-600' : 'text-blue-600') }}">
                                                        {{ $this->formatPriceDifference($cosmashopDifference) }}
                                                    </span>
                                                </div>
                                            </div>
                                        ...
                                        <!-- Volumes (si disponibles) -->
                                        @if(!empty($productVolumes))
                                        <div class="pt-2">
                                            <div class="text-xs text-gray-500 mb-1">Volumes:</div>
                        ...
                        @else
                            ...
                            <div class="text-center py-12 bg-white rounded-lg border border-gray-300">
                                ...
                                @if(array_filter($filters))
                                <button wire:click="resetFilters" 
                                        class="mt-4 inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded-md shadow-sm text-white bg-red-600 hover:bg-red-700">
                                    Réinitialiser les filtres
                                </button>
                                @endif
                            </div>
