...
class Boutique extends Component
{
    ...
    // Préfixe pour toutes les clés de cache de la boutique
    private $cachePrefix = 'boutique_';
    ...
    {
        return $this->cachePrefix . sprintf(
            'products_%s_%s_%s_%s_%s_%d_%d',
            md5($search),
            md5($this->filterName),
    ...
    /**
     * Génère une clé de cache pour le comptage
     */
    private function getCountCacheKey($search, $filtersHash)
    {
        return $this->cachePrefix . 'count_' . md5($search . $filtersHash);
    }

    /**
     * Génère un hash pour les filtres actuels
     */
    private function getFiltersHash()
    {
        return md5($this->filterName . $this->filterMarque . $this->filterType . $this->filterCapacity);
    }

    /**
     * Enregistre une clé de cache dans la liste des clés utilisées
     */
    private function registerCacheKey($key)
    {
        $keys = Cache::get($this->cachePrefix . 'registered_keys', []);
        if (!in_array($key, $keys)) {
            $keys[] = $key;
            // Stocker la liste pour 60 jours
            Cache::put($this->cachePrefix . 'registered_keys', $keys, now()->addDays(60));
        }
    }
    ...
    {
        // Méthode 1: Effacer toutes les clés de la boutique
        $this->clearBoutiqueCache();
        
        // Méthode alternative: Effacer tout le cache (plus radical)
        // Cache::flush();
        
        $this->dispatch('notify', message: 'Cache de la boutique effacé avec succès!');
    }

    /**
     * Efface uniquement le cache de la boutique
     */
    private function clearBoutiqueCache()
    {
        $keys = Cache::get($this->cachePrefix . 'registered_keys', []);
        ...
        // Effacer aussi la liste des clés
        Cache::forget($this->cachePrefix . 'registered_keys');
    }
    ...
    public function getListProduct($search = "", $page = 1, $perPage = null)
    {
        ...
        // Enregistrer la clé pour pouvoir la nettoyer plus tard
        $this->registerCacheKey($cacheKey);

        // Tenter de récupérer depuis le cache
        return Cache::remember($cacheKey, now()->addMinutes($this->cacheDuration), function () use ($search, $page, $perPage) {
    ...
    private function fetchProductsFromDatabase($search = "", $page = 1, $perPage = 12)
    {
        try {
            ...
            // Générer une clé de cache pour le comptage basée sur les filtres actuels
            $filtersHash = $this->getFiltersHash();
            $countCacheKey = $this->getCountCacheKey($search, $filtersHash);
            $this->registerCacheKey($countCacheKey);
            
            $total = Cache::remember($countCacheKey, now()->addMinutes($this->cacheDuration), function () use ($subQuery, $params) {
    ...
    /**
     * Méthode pour invalider le cache quand un produit est modifié
     * À appeler depuis d'autres parties de l'application
     */
    public static function invalidateCache()
    {
        $instance = new static();
        $instance->clearBoutiqueCache();
    }
