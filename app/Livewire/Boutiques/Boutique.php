...
class Boutique extends Component
{
    ...
    // Durée du cache en minutes (60 = 1 heure, 1440 = 1 jour)
    ...
    public function getListProduct($search = "", $page = 1, $perPage = null)
    {
        ...
        return Cache::tags(['products'])->remember($cacheKey, $this->cacheDuration * 60, function () use ($search, $page, $perPage) {
     ...
     */
    private function fetchProductsFromDatabase($search, $page, $perPage)
    {
        try {
            ...
            // Total count (également mis en cache)
            $countCacheKey = $this->getCacheKey($search . '_count', 1, 1);
            $total = Cache::tags(['products'])->remember($countCacheKey, $this->cacheDuration * 60, function () use ($subQuery, $params) {
