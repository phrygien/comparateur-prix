...
class Boutique extends Component
{
     ...
     */
    private function getCountCacheKey($subQuery, $params)
    {
        return 'boutique_count_' . md5($subQuery . serialize($params));
    }
    ...
    {
        // Solution 1: Si vous utilisez un driver qui supporte les tags
        // Cache::tags(['boutique_products'])->flush();
        
        // Solution 2: Effacer manuellement les clés de cache
        // Vous pouvez implémenter une logique pour suivre toutes les clés de cache
        // créées et les effacer une par une
        
        // Solution 3: Éviter d'effacer tout le cache, ou effacer tout le cache
        Cache::flush(); // Attention: cela efface TOUT le cache de l'application
    ...
    public function getListProduct($search = "", $page = 1, $perPage = null)
    {
        ...
        // Tenter de récupérer depuis le cache (sans tags)
    ...
    private function fetchProductsFromDatabase($search = "", $page = 1, $perPage = 12)
    {
        try {
            ...
            // Total count avec cache séparé (sans tags)
            $countCacheKey = $this->getCountCacheKey($subQuery, $params);
